...
namespace ExpertPlus.Items.RetinazersTowerShield{
	[AutoloadEquip(EquipType.Shield)]
	public sealed class RetinazersTowerShield : ModItem{
		public static readonly string InlineWikiLibValue = @"
## Retinazers Tower Shield ![ExpertPlus/Items/RetinazersTowerShield]t4

The Retinazers TowerS hield Collar Grants 15% damage reduction and 6 defense.
";
		...
		public override void UpdateAccessory(Player player, bool hideVisual){
			...
			player.GetModPlayer<RetinazersDashPlayer>().DashAccessoryEquipped = true;
	...
	public class RetinazersDashPlayer : ModPlayer{
		...
		public override void PreUpdateMovement() {
			...
			if (DashTimer > 0) { // dash is active
				...
				foreach (NPC npc in Main.npc) {
					if (Player.Hitbox.Intersects(npc.Hitbox)) {
						// 1️⃣ Damage Calculation
							...
							npc.SimpleStrikeNPC(60, 1, crit, 1, DamageClass.Melee); // Apply damage

						// 2️⃣ Spawn the Projectile
							if (Main.myPlayer == Player.whoAmI) { // Only the client who owns the player should spawn the projectile
								Vector2 shootDirection = (npc.Center - Player.Center).SafeNormalize(Vector2.UnitX); // Aim at enemy
								Vector2 projectileVelocity = shootDirection * 10f; // Adjust speed if needed

								Projectile.NewProjectile(
								Player.GetSource_FromThis(), // Source
									Player.Center, // Spawn at player
									projectileVelocity, // Velocity
									ProjectileID.DeathLaser, // Projectile Type (Retinazer laser)
									10, // Damage
									5f, // Knockback
									Player.whoAmI // Owner
								);
				...
				// 3️⃣ Make Player Temporarily Immune
				Player.immune = true;
